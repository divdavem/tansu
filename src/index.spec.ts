...
describe('stores', () => {
  describe('base', () => {
    ...
    it('should accept the oldSubscription parameter (without any change while unsubscribed)', () => {
      const store = writable(0);
      const values: number[] = [];
      const storeValue = (value: number) => values.push(value);
      const subscription1 = store.subscribe(storeValue);
      store.set(2);
      subscription1.unsubscribe();
      const subscription2 = store.subscribe({
        next: storeValue,
        oldSubscription: subscription1,
      });
      store.set(3);
    ...
    it('should accept the oldSubscription parameter (with a reverted change while unsubscribed)', () => {
      const store = writable(0);
      const values: number[] = [];
      const storeValue = (value: number) => values.push(value);
      const subscription1 = store.subscribe(storeValue);
    ...
    it('should accept the oldSubscription parameter (with a change while unsubscribed)', () => {
      const store = writable(0);
      const values: number[] = [];
      const storeValue = (value: number) => values.push(value);
      const subscription1 = store.subscribe(storeValue);
