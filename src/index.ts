...
export function asReadable<T, U = object>(
  ...
  const subscribe = getNormalizedSubscribe(input);
...
export abstract class Store<T> implements Readable<T> {
  ...
  subscribe(subscriber: Subscriber<T>): UnsubscribeFunction & UnsubscribeObject {
    ...
    const oldSubscription = subscriber?.oldSubscription;
    if (oldSubscription) {
      const oldSubscriberObject = this.#oldSubscriptions.get(oldSubscription);
