...
const interopSignalMap = new WeakMap<RawStore<unknown, BaseLink<unknown>>, WatchableSignal>();

export const toInteropSignal = <T, Link extends BaseLink<T>>(
  producer: RawStore<T, Link>
): WatchableSignal => {
  let res = interopSignalMap.get(producer);
  if (!res) {
    res = {
      watchSignal(notify) {
        const watcher = new WatcherConsumer(producer, notify);
        return {
          isUpToDate: watcher.isUpToDate.bind(watcher),
          isStarted: watcher.isStarted.bind(watcher),
          update: watcher.update.bind(watcher),
          start: watcher.start.bind(watcher),
          stop: watcher.stop.bind(watcher),
        };
      },
    };
    interopSignalMap.set(producer, res);
  }
  return res;
};
