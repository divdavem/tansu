export interface ConsumerLibrary {
  onActiveChange?(isActive: boolean, restore: boolean): void;
  hasActiveConsumer?(): boolean;
  producerAccessed(signal: WatchableSignal): void;
}

interface SignalInteropLib {
  setActiveConsumerLibrary(library: ConsumerLibrary): () => void;
  hasActiveConsumer(): boolean;
  producerAccessed(producer: WatchableSignal): void;
  beginBatch: () => () => { error: any } | void;
  afterBatch: (fn: () => void) => void;
}

const createSignalInterop = (): SignalInteropLib => {
  let activeConsumerLibrary: ConsumerLibrary | null = null;
  let inBatch = false;
  let batchQueue: (() => void)[] = [];
  ...
  return {
    setActiveConsumerLibrary(library) {
      if (activeConsumerLibrary === library) {
        return noop;
      }
      let active = true;
      const prevLibrary = activeConsumerLibrary;
      prevLibrary?.onActiveChange?.(false, false);
      activeConsumerLibrary = library;
      library.onActiveChange?.(true, false);
      return () => {
        if (active && activeConsumerLibrary === library) {
          active = false;
          library.onActiveChange?.(false, true);
          activeConsumerLibrary = prevLibrary;
          prevLibrary?.onActiveChange?.(true, true);
        }
      };
    },
    hasActiveConsumer: () =>
      activeConsumerLibrary ? (activeConsumerLibrary.hasActiveConsumer?.() ?? true) : false,
    producerAccessed: (producer) => {
      activeConsumerLibrary?.producerAccessed(producer);
    },
    beginBatch,
    afterBatch: (fn) => {
      if (inBatch) {
...
const signalInterop: SignalInteropLib = (() => {
  let res: SignalInteropLib | undefined = (globalThis as any).signalInterop;
  if (!res) {
...
/**
 * An object exposing a signal in an interoperable way.
 */
...
/**
 * Set the active consumer library.
 *
 * @returns a function to call to restore the previous active consumer library.
 */
export const setActiveConsumerLibrary = signalInterop.setActiveConsumerLibrary;

/**
 * Return whether there is an active consumer.
 */
export const hasActiveConsumer = signalInterop.hasActiveConsumer;

/**
 * Notify that a producer was accessed.
 */
export const producerAccessed = signalInterop.producerAccessed;
